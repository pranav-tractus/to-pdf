...
program
  .name("to-pdf")
  .description("Convert DOCX and image files to PDF")
  ...
  .action(async (files: string[], options: { output?: string; outputDir?: string }) => {
    if (files.length === 0) {
    ...
    if (files.length > 1 && options.output) {
      console.error("Use --output-dir for multiple files.");
      process.exit(1);
    }
